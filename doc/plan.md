...
- The PoC implements the conditional three-digit ZIP rule with a versioned, default-deny allowlist. A ZIP prefix is retained only when an approved current Census evidence record confirms that the combined geographic unit for the prefix has more than 20,000 people; otherwise the ZIP becomes 000. No allowlist is currently approved, so the synthetic preview defaults to 000 for every recognized ZIP.
...
**Source:** Approved .xlsx workbook with Clinical_Data and the initial eight-column contract:
...
| Phase 7 — PoC execution | Blocked | The Bootstrap 5 UI banner, configuration, and export guard keep the application synthetic-only and non-releasing. Shiny safety suite passed on 2026-08-21. | Do not run PHI until every preceding gate is complete and approved. |

### Review Follow-ups

- [ ] Obtain, version, and approve the current Census three-digit ZIP population allowlist before any eligible prefix is retained; the default-deny policy currently transforms every recognized ZIP to 000.
...
| 2026-08-18 | Uploaded workbook verification | `Clinical_PHI_Anonymization_Data.xlsx` processed successfully; empty drawing containers were accepted and `Zip_Code` was generalized. |
| 2026-08-18 | `tests/run-core-tests.R` | Passed: 30 tests, 0 failures, including empty drawing containers and numeric/non-U.S. postal values. |
...
| 2026-08-18 | Local free-text preparation inspection of the user-supplied workbook | Read only `Diagnosis_Journey` and `Treatment_History` into process memory; 20 non-empty documents, 3,030 original and 2,548 locally prepared Unicode code points, and 20 estimated Azure text records both before and after preparation. No Azure request, output workbook, log of cell content, or PHI-derived fixture was created. This user-directed local check does not satisfy Phase 0 approval or authorize PHI transmission. |
| 2026-08-18 | `tests/run-core-tests.R` and `tests/run-app-tests.R` | Passed: 38 core tests, 0 failures; Shiny milestone safety tests passed. Coverage includes deterministic/known-value preparation, Unicode-codepoint chunk maps, text-record estimation, post-redaction local residual detection, and the clearly labelled deterministic-preview detection-details table. |
| 2026-08-21 | Bootstrap 5 UI modernization and `tests/run-core-tests.R` / `tests/run-app-tests.R` | Replaced the legacy Shiny page/sidebar containers with bslib theming, sidebar, cards, accordion, and task button while preserving the same section order and synthetic-only warnings. The worksheet selector is now persistent and updated server-side rather than recreated with `renderUI()`. Passed: 42 core tests, 0 failures; Shiny milestone safety tests passed. This is PoC UI evidence only and does not change the blocked release state. |
| 2026-08-21 | Synthetic attestation control removal | Removed the synthetic-data confirmation checkbox and its validation, processing, download, test, and run-guide dependencies at user direction. The persistent synthetic-only banner, `mode: synthetic_only`, disabled Azure configuration, non-releasable preview labels, and blocked release path remain unchanged. No additional regression suite was run for this follow-up at user request. |
| 2026-08-21 | Synthetic preview layout and controls | Moved the conditional synthetic-preview download ahead of the data table and approved contract, labelled the requested anonymized-data table as a synthetic-only unvalidated preview, converted processing and download actions to accessible icon-only controls with tooltips, and removed the processed-run status alert while retaining processing-error feedback. Passed: 42 core tests, 0 failures; Shiny milestone safety tests passed. Release controls and synthetic-only safety messaging remain unchanged. |
| 2026-08-21 | Default workbook source and compact preview header | Added a Bootstrap 5 radio source selector that defaults to `Clinical_PHI_Anonymization_Data.xlsx` and automatically processes its `Clinical_Data` sheet at launch; upload mode retains explicit XLSX and worksheet selection. Consolidated the synthetic preview download and table into one card so a single accessible download icon sits in the header directly above the table. The workbook `Test_Guide` sheet was not treated as instructions. Passed: 42 core tests, 0 failures; Shiny milestone safety tests passed, including default-load, single-icon, and output-order checks. |
| 2026-08-21 | Pill data-source navigation and page-wide synthetic warning | Replaced the radio source selector and conditional panels with a two-state Bootstrap 5 pill navigation whose explicit `default` and `upload` values preserve existing server behavior. Moved the synthetic-only warning above the sidebar layout, changed it to yellow warning styling, and added an accessible dismiss control; dismissing the notice does not change safety or release state. Passed: 42 core tests, 0 failures; Shiny milestone safety tests passed, including pill values, warning placement, and dismissible-alert markup. `air` was unavailable, so parse and `git diff --check` were used as formatting/static fallbacks. |
| 2026-08-21 | Release-status sidebar section removal | Removed the redundant `Release enabled: No` sidebar section while retaining the `Release unavailable` explanation and all fail-closed configuration and export guards. Added source and rendered-UI regression assertions that the removed label is absent. Passed: 42 core tests, 0 failures; Shiny milestone safety tests passed. |
| 2026-08-21 | Synthetic badge removal | Removed the `Synthetic only` title badge and `Synthetic preview only - not validated` table-header badge at user direction. Retained the page-wide synthetic demonstration warning, preview limitations, non-release explanation, and fail-closed controls. Updated source/rendered-UI regression checks and run-guide expectations. Passed: 42 core tests, 0 failures; Shiny milestone safety tests passed. |
| 2026-08-21 | Warning color correction and release-alert removal | Applied explicit Bootstrap warning color variables to keep the page-wide synthetic demonstration notice yellow, and removed the redundant `Release unavailable` alert from the validation-blockers card. The underlying release configuration and export guards remain unchanged. Added rendered-style and message-absence regression checks. Passed: 42 core tests, 0 failures; Shiny milestone safety tests passed. |
| 2026-08-21 | Whole-page and table-card busy indicators | Initialized Shiny busy indicators once at the top level of the returned UI, before the warning and bslib page, and added card-scoped dots indicators to all four table cards, including fade behavior during recalculation. Added source and rendered-UI checks for single whole-page spinner/pulse activation, initialization order, and four card configurations. Passed: 42 core tests, 0 failures; Shiny milestone safety tests passed. |
